...
async function reqXmDirect(songId, quality) {
  // 喜马拉雅 track detail API — 获取单集音频播放地址
  // 参考：https://open.ximalaya.com/doc
  const ts = Math.floor(Date.now() / 1000)
  const url = `https://api.ximalaya.com/mobile/v1/track/trackInfo/ts-${ts}?trackId=${encodeURIComponent(String(songId))}&device=android`
  ...
  const body = res.data
  if (body?.ret !== 0) throw new Error('喜马拉雅API错误: ' + (body?.msg || 'ret=' + body?.ret))
  const data = body?.data
  if (!data) throw new Error('喜马拉雅无数据')

  // 音质映射：128k → playUrl64, 64k → playUrl32, 32k → playPath32
  const qualityFieldMap = { '128k': 'playUrl64', '64k': 'playUrl32', '32k': 'playPath32' }
  const field = qualityFieldMap[quality]
  let playUrl = field ? data[field] : null

  // 降级：逐个尝试可用音质
  if (!playUrl) {
    playUrl = data.playUrl64 || data.playUrl32 || data.playPath64 || data.playPath32
  }
  if (!playUrl) throw new Error('喜马拉雅无可用播放地址')

  return { url: playUrl, quality, apiName: '喜马拉雅直连' }
}
