...
export default {
  ...
  albumSearch(str, page, limit) {
    const searchRequest = httpFetch(`https://m.music.migu.cn/migu/remoting/scr_search_tag?keyword=${encodeURIComponent(str)}&type=4&pgc=${page}&rows=${limit}`, {
  ...
  filterData(rawData) {
    ...
    rawData.forEach(item => {
      ...
      list.push({
        ...
        img: item.albumPicM || item.albumPicL || item.albumPic || item.img || null,
        source: 'mg',
        publish_date: item.publishDate || item.publish_date || '',
        song_count: item.fullSongTotal || item.songCount || item.song_count || 0,
      })
    })
    return list
  },
  ...
  search(str, page = 1, limit, retryNum = 0) {
    ...
    return this.albumSearch(str, page, limit).then(result => {
      ...
      let list = this.filterData(result.albums || [])
