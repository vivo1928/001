...
export default {
  ...
  filterData(rawData) {
    ...
    rawData.forEach(item => {
      if (!item.singerId || ids.has(item.singerId)) return
      ...
      list.push({
        ...
        album_count: item.albumCount || item.album_count || 0,
      })
    })
    return list
  },
  ...
  search(str, page = 1, limit, retryNum = 0) {
    ...
    return this.singerSearch(str, page, limit).then(result => {
      ...
      const singerResultData = result.singerResultData || { resultList: [], totalCount: 0 }
      let list = this.filterData(singerResultData.resultList || [])
