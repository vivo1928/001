...
const filterData = (rawList) => {
  ...
  return rawList.map(item => {
    ...
    return {
      id: String(item.id),
      name: item.name || '',
      singer: singer || item.artistName || item.djName || item.dj?.nickname || '',
      img: item.picUrl || item.coverImgUrl || item.blurPicUrl || item.coverUrl || '',
      source: 'wy',
      song_count: item.size || item.listenerCount || 0,
...
const search = async (str, page = 1, limit = 20, retryNum = 0) => {
  ...
  try {
    ...
    const rawList = result.result ? (result.result.programs || []) : []
    const list = filterData(rawList)
    const total = result.result ? (result.result.programCount || 0) : 0
