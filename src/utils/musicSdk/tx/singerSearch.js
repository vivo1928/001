...
const filterData = (rawList) => {
  return rawList.map(item => ({
    ...
    img: `https://y.gtimg.cn/music/photo_new/T001R300x300M000${item.singer_mid}.jpg`,
    source: 'tx',
...
const handleResult = (rawList) => {
  ...
  return rawList.filter(item => {
    if (seen.has(item.singer_mid)) return false
    seen.add(item.singer_mid)
    return true
  })
}
...
const search = async (str, page = 1, limit = 20, retryNum = 0) => {
  ...
  try {
    ...
    const rawList = data ? data.singer_list || [] : []
